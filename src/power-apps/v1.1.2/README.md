# Employee Recognition Canvas App (v1.1.2)
...
- **Triage Workflow Overhaul:** Complete redesign of Triage_Screen with dynamic form visibility, team routing combo box, and comprehensive state cleanup on submit
- **Office365 API Migration:** Updated from `SearchUserV2` to `SearchUser` for user profile lookups
- **Platform Modernization:** Upgraded MSAppStructureVersion from 2.0 to 2.4.0; groupContainer control from 1.3.0 to 1.4.0
- **Enhanced Audit Trails:** Auto-populated "Action By" field with current user email
- **Grid Layout Support:** Added LayoutGridColumns/LayoutGridRows properties to containers
...
```
src/power-apps/v1.1.2/
  README.md                      # This consolidated document
  app-config.json                # UI + feature + validation + branding config
  v1.1.2_ReleaseMessage.md       # Detailed release notes and change analysis
  .msapp/                        # Packaged app artifacts
  .unpacked/                     # Unpacked source for version control
    Src/                         # Power Fx YAML source files
    DataSources/                 # Data source configurations
    Connections/                 # Connection definitions
    pkgs/                        # Control packages and templates
```

Packaged artifacts (.msapp) are stored under release directories using `pac canvas pack`. Unpacked sources stay in versioned folders and are committed for diff-based review.
...
| **Triage (NEW v1.1.2)** | Dynamic form visibility, combo box team routing, auto-populated action tracking      |
...
| -------------------------------- | ------------------------------ | ---------------------------------------------------------------------------- | --------------------------------------------------------------- |
| Loading                          | App initialization             | Version display, timer-based navigation, form resets                         | Entry hub; triggers OnTimerEnd/Start for state setup            |
| Submit Recognition               | New nomination form            | Employee selector, award level/type, description, justification, attachments | Draft + validation; dynamic question sets per award type        |
| My Nominations (ViewSubmissions) | User submissions gallery       | Filters (status/date/name), edit drafts, toolbar quick-submit buttons        | Delegation-aware filtering; toolbar navigates to new submission |
| **Triage (v1.1.2)**              | Team assignment workflow       | **Dynamic form visibility**, combo box team routing, auto "Action By"        | Redesigned with state cleanup on submit                         |
...
| Update/Help                      | User assistance                | Popup dialogs, external links, tutorial references                           | VA IT service integration links                                 |
...
1. Download `EmployeeRecognitionApp_v1.1.2.msapp` from `src/power-apps/v1.1.2/.msapp/`.
...
| Version          | Highlights                                                                                                                                                                                                       |
| ---------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| v1.1.2 (Current) | **Triage workflow overhaul:** dynamic form visibility, combo box team routing, auto "Action By" field; Office365 API migration to SearchUser; MSAppStructureVersion 2.4.0; groupContainer 1.4.0 with grid layout |
| v1.0.6           | Toolbar submission flow refinements; triage form/gallery binding; preliminary version metadata groundwork                                                                                                        |
