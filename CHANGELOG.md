...
- **Triage workflow enhancements:** Redesigned triage screen with dynamic form visibility tied to selection state, guided team routing via combo box options, and a dedicated submit action with comprehensive state cleanup.
- **Portable YAML artifacts:** Added/retained `.pa.yaml` files under `Other/Src/` as part of newer Power Apps source conventions.

### Changed

- **Office365Users connector:** Migrated user lookup from `Office365Users.SearchUserV2` to `Office365Users.SearchUser`.
- **Platform/manifest alignment:** Updated platform metadata (including MSAppStructureVersion) and upgraded control packages (e.g., `groupContainer`) enabling grid layout properties.
- **Audit behavior:** Auto-populated triage action tracking fields using the current user context.

### Removed

- **Preview/Copilot flags:** Disabled/removed several preview flags in the app manifest, including Copilot preview controls.

### Notes

- Detailed, line-level diff notes are maintained in `src/power-apps/v1.1.2/v1.1.2_ReleaseMessage.md`.
